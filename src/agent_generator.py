...
def generate_agents(env: Network, num_agents: int, threshold=None, random_seed:int=0, start_positions=None) -> List[Agent]:
    """
    ...
    # If start_positions is None, generate the start positions
    if start_positions is None:
        # Generate the agents
        agents = [Agent(env, agent_id, start_pos, threshold) for agent_id, start_pos in generate_start_positions(env, num_agents)]
        return agents
        
    agents = [Agent(env, agent_id, start_pos, threshold) for agent_id, start_pos in assign_start_positions(env, num_agents, start_positions)]
    # Return the agents
    return agents
...
def assign_start_positions(env: Network, num_agents: int, start_positions: List[str]):
    """
    Method to assign agents to given start positions
    ...
    num_agents: int
        number of agents to generate
    start_pos: list of nodes
        list of nodes to assign agents to
    ...
    # Assign the start positions to the agents - round robin style so agents are evenly distributed across the start positions
    agent_pos = [(agent_id, start_positions[agent_id % len(start_positions)]) for agent_id in range(num_agents)]
